...
> 当前基线（2026-08-03 更新）：`npm test` **166/166 通过**（11 个测试文件），`npm run build` 成功（PWA 预缓存 13 项）。Round 3 commit `f10ffe6`、亚古兽去背/推荐缺陷修复 commit `4933d72` 与裁剪像素画智能尺寸修复 commit `88e68d3` 均已推送并发布，见下方「0.5 实施状态总览」。
...
>
> **裁剪像素画智能尺寸修复（2026-08-03，已发布）**：修复非整数缩放与倍频误判导致实际角色从 23×31 错缩为 11×15，以及推荐分类与转换分类不一致的问题；测试 **166/166**、生产构建与本地/线上实际裁剪图回归通过。commit `88e68d3` 已推送并原子发布，回滚备份 `/var/www/image2pindou.backup-20260803-222001`。
...
- **4.6 草稿管理 UI**、**5.3 IndexedDB 草稿持久化**（刷新不丢）
- **2.2 自动抠图增强剩余项**（封闭纯色孔洞与透明黑底已修复；独立上传入口、复杂掩码中心先验仍待做）、**2.6 语义颜色保护**
...
- **实施**：自动显著度焦点与手动点击/方向键焦点已接入 cover 重采样；窗口按焦点附近黄金分割定位并钳制在图像边界内，手动框选优先。
...
- **实施**：新增全部/项目/草稿 segmented control；统计、库存差缺、首页计数和采购 CSV 均跟随所选口径。
...
- **2026-09-07 角色原图精细图纸（已发布）**：新增按主体比例多档尺寸、精细采样与特征色保护、独立图片设置/尺寸副本、完整高清图纸清单；修复固定步长漏检像素边界与取消迟到结果。全量 181/181、生产构建及真实角色/375px 浏览器回归通过。`a51df85` 已推送并部署至 VPS；备份 `/var/www/image2pindou.backup-20260907-004236`，Nginx、哈希和公网资源检查及线上 156×138 精细生成通过。使用入口、验证与局限见 `docs/ITERATIONS.md` §1.11 和 `docs/DEVLOG.md` §5.3。
...
- **裁剪像素画智能尺寸修复发布记录（2026-08-03）**：commit `88e68d3` 已推送并部署；主入口 `index-BCt6leN2.js`、CSS `index-BGRK9mwY.css`、转换 Worker `conversion.worker-pk1enJnj.js` 与 PWA 文件全部 200。生产站实际裁剪图恢复为 23×31，推荐为像素锐利，应用后尺寸保持不变，console error 为 0；回滚备份 `/var/www/image2pindou.backup-20260803-222001`。
