...
>
> 当前本地基线：`npm test` **156/156**（11 文件）、`npm run build` 成功；Round 3 已完成但尚未发布。线上 `https://pindou.fanni-panda.com` 仍为 Round 2（VPS 部署，见 `docs/DEVLOG.md`）。
...
### 1.4 cover/contain 焦点控制（P1，✅ 2026-08-02 本地完成，待发布）
...
### 1.9 草稿与项目计数口径（P1，✅ 2026-08-02 本地完成，待发布）
...
- **现状**：复杂背景自动走 `@imgly/background-removal`（WASM，24MB 懒加载），简单纯色背景走 `src/domain/background.ts` 自适应泛洪；上传缩略图无「一键抠图」入口，且无显著度中心先验。
...
### 2.5 自动描边建议（P2，✅ 2026-08-02 本地完成，待发布）
